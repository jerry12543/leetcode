...
class Solution {
    ...
    std::vector<std::unordered_map<uint32_t, bool>> rows;
    std::vector<std::vector<bool>> cols;
    std::vector<std::vector<bool>> squares;

    std::deque<std::pair<uint32_t, uint32_t>> empties;
    std::deque<std::pair<uint32_t, uint32_t>> updateOrder;

    // Returns the box the given cell resides in
    [[nodiscard]] auto currentSquare(uint32_t row, uint32_t col) -> std::vector<bool>& {
